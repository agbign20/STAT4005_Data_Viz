...
We would want to construct a static graph instead of an interactive graph when trying to show one particular thing like other majors of stat majors for example in the SLU Majors data set. The static graph would be better since it would be focusing on one particular thing we want to look at.
...
server <- function(input, output, session) {
  ...
  box_plot <- reactive({
    ggplot(df_sub(), aes(x =.data[[input$varchoice]])) + geom_boxplot(colour = "black", fill = "white")
  })
