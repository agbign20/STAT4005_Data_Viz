...
state_full <-
  state_full %>%
  mutate(percent_change_pop = 100 * (pop2010 - pop2000) / pop2000) 

ggplot(data = state_full, aes(x = long, y = lat, group = group)) +
  geom_polygon(colour = "black", aes(fill = percent_change_pop)) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  theme_void() +
  scale_fill_viridis_b()
```
Nevada has the fastest growth rate between 2000 and 2010.
...
# brewer is for continuous variable
```
